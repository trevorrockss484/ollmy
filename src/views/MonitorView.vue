<template>
  <div class="monitor-page">
    <div class="page-header">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;">
        ...
        <div style="display:flex;gap:8px;align-items:center;">
          <el-tag v-if="counts.overdue" type="danger" size="large" effect="dark" round>⚠️ {{ counts.overdue }} 已过期</el-tag>
    ...
    <div class="vps-overview-bar">
      <div class="vps-ov-item danger" v-if="counts.overdue">
        <div class="vps-ov-icon">⚠️</div><div class="vps-ov-info"><div class="vps-ov-num">{{ counts.overdue }}</div><div class="vps-ov-label">已过期</div></div>
      </div>
      <div class="vps-ov-item urgent" v-if="counts.urgent">
        <div class="vps-ov-icon">🔴</div><div class="vps-ov-info"><div class="vps-ov-num">{{ counts.urgent }}</div><div class="vps-ov-label">7天内到期</div></div>
      </div>
      <div class="vps-ov-item ok">
        <div class="vps-ov-icon">🟢</div><div class="vps-ov-info"><div class="vps-ov-num">{{ counts.ok }}</div><div class="vps-ov-label">运行正常</div></div>
      </div>
      <div class="vps-ov-item" v-if="costUnlocked">
        <div class="vps-ov-icon">💵</div><div class="vps-ov-info"><div class="vps-ov-num" style="color:#10b981;">¥{{ profit.total.toFixed(2) }}</div><div class="vps-ov-label">月利润</div></div>
      </div>
      <div class="vps-ov-item" v-if="costUnlocked">
        <div class="vps-ov-icon">📦</div><div class="vps-ov-info"><div class="vps-ov-num" style="color:#6366f1;">¥{{ profit.totalCost.toFixed(2) }}</div><div class="vps-ov-label">月成本</div></div>
      </div>
    </div>
    ...
    <div class="vps-toolbar">
      <div class="vps-toolbar-left">
        ...
        <span style="font-size:12px;color:#9ca3af;">共管理 <b style="color:#6366f1;">{{ list.length }}</b> 台</span>
        <el-input
          v-model="searchText"
          placeholder="🔍 搜索VPS名称、服务商、备注..."
          size="small"
          clearable
          style="width:260px;"
        />
      ...
      <div class="vps-toolbar-right">
        <el-button-group size="small">
          <el-button v-for="f in statusFilters" :key="f.key"
            :type="filterType === f.key ? 'primary' : ''"
            @click="filterType = (filterType === f.key ? 'all' : f.key)"
          >{{ f.label }} <span v-if="filterType !== f.key && countByType(f.key)" style="opacity:.5;">{{ countByType(f.key) }}</span></el-button>
        </el-button-group>
        <el-select v-model="filterProvider" size="small" placeholder="服务商" clearable style="width:150px;">
          <el-option v-for="p in providerOptions" :key="p" :label="p" :value="p" />
        </el-select>
        <el-select v-model="filterCountry" size="small" placeholder="国家/地区" clearable style="width:140px;" filterable>
    ...
    <transition name="slide">
      <div v-if="formOpen" class="vps-form-card">
        ...
        <div style="display:flex;gap:8px;margin-top:16px;">
          <el-button type="primary" size="large" @click="addVps">💾 保存VPS</el-button>
        </div>
      </div>
    </transition>
      ...
      >
        ...
        <template v-if="editingId === v.id">
          ...
          <div class="vps-card-actions">
            <el-button type="primary" size="small" round @click="doUpdateVps(v.id)">💾 保存</el-button>
        ...
        <div class="vps-card-details">
          <div class="vps-detail" v-if="v.url">
            <span class="vps-detail-label">🔗</span>
        ...
        <div v-if="v.warnText" class="vps-alert" :class="v.severity">
          <span class="vps-alert-icon">{{ v.severity === 'overdue' ? '‼️' : '⚠️' }}</span>
        ...
        <div class="vps-card-actions">
          ...
          <el-button size="small" round type="primary" plain @click="editVps(v)">✏️ 编辑</el-button>
          <el-button size="small" round type="danger" plain @click="remove(v)">🗑 删除</el-button>
        </div>
        </template>
      </div>
    </div>
...
function defaultIcon(v) { return '' }

// buildVpsCard — 按月周期进度条
function buildVpsCard(v) {
  const daysLeft = daysBetween(todayStr(), v.expire)
  const expireDate = new Date(v.expire)
  // 月付周期：到期日往前推30天
  const cycleDays = 30
  const startDate = new Date(expireDate)
  startDate.setDate(startDate.getDate() - cycleDays)
  const totalSpan = daysBetween(startDate.toISOString().split('T')[0], v.expire) // ~30天
  const elapsed = daysBetween(startDate.toISOString().split('T')[0], new Date().toISOString().split('T')[0])
...
function countByType(key) {
  return filteredList.value.filter(v => {
...
async function addVps() {
  ...
  const expire = typeof form.expire === 'string' ? form.expire : form.expire
  const res = await api.vps.add({ name: form.name, type: form.type, country: form.country, icon: form.icon, provider: form.provider, url: form.url, expire, costPrice: form.costPrice, sellPrice: form.sellPrice, price: form.price, status: form.status, note: form.note })
  if (res.success) {
...
.vps-ov-item.warning { border-color:#fed7aa; background:linear-gradient(135deg,#fffcf5,#fff); }
...
.vps-form-card.editing {
  border-color:#6366f1; box-shadow:0 0 0 3px rgba(99,102,241,.1), 0 8px 32px rgba(99,102,241,.1);
}
.vps-form-editing-banner {
...
/* === VPS 卡片网格 === */
.vps-grid { display:grid; gap:20px; }
...
.vps-timeline-deadline {
  position:absolute; top:0; bottom:0;
  width:3px; background:#ef4444;
  box-shadow:0 0 10px rgba(239,68,68,.5), 0 0 20px rgba(239,68,68,.2);
